...
## Our World In Data: Monkey Pox Worldwide

```{r}
rm(list=ls())
monkey_pox <- read_csv("https://raw.githubusercontent.com/owid/notebooks/main/EdouardMathieu/monkeypox/owid-monkeypox-data.csv") %>% janitor::clean_names()
dim(monkey_pox)
```
```{r}
summary(monkey_pox)
...
```{r}
worldwide_daily %>% ggplot() + geom_line(aes(x=date,y=Deaths)) +
  labs(title="Worldwide Deaths by Date")
```
```{r}
worldwide_daily$Cumdeaths <- cumsum(worldwide_daily$Deaths)
```
```{r}
worldwide_daily %>% ggplot() + geom_line(aes(x=date,y=Cumdeaths)) +
  labs(title="Worldwide Cumulative Total Deaths by Date")
```
